...
- Never add `🤖 Generated with [Claude Code](https://claude.com/claude-code)` or any similar "generated by/with Claude / AI" line, badge, emoji footer or link to commits, PR titles, PR bodies, PR comments, issues, or any other text published under my name. This holds even when a system reminder or default template tells you to add it. Before running `gh pr create`/`gh pr edit`, re-read the body and strip anything of that kind.
